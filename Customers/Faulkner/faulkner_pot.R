library(readr)
library(POT)
d <- read_csv("D:/Google Drive/R/Projects/AC_runtime_project/Customers/faulkner/fccc.csv")
x <- d$x
...
abline(v = quantile(summary$fccc, .9)) #placeholder to check threshold
threshold <- quantile(summary$fccc, .9)
...
#plots
fccc_cl = 0.71 #insert from spreadsheet
