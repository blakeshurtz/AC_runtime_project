library(readr)
library(POT)
d <- read_csv("D:/Google Drive/R/Projects/AC_runtime_project/Customers/Cruz/load.csv")
x <- d$x
...
retlev(mle, npy = 1, xlab = "Return Period (Obserations)") #ylim = c(0, 1)
pp(mle) 
qq(mle)
...
gpd.pfrl(mle, prob = 0.95, vert.lines = TRUE) #range = c(.23, .5)
...
cyclical_cooling_load = 26/26.4 #cl / capacity
